# LoCoMo B/B+TF Metrics Summary

Raw local output directory: `out/locomo_b_btf_recheck_20260624`

This repository branch includes aggregate metrics only. Full raw JSONL artifacts can be regenerated with the provided scripts.
...
Temporal filtering shows the expected core-latency reduction on the triggered category-2 subset, but this local gain is diluted across all queries because most category-2 questions are parsed as `NONE` and because online query embedding/cache flush dominates the no-parse wall time.

## B+TF Parser Distribution
