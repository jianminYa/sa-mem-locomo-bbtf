...
class EnhancedRetriever:
    ...
    def __init__(
        ...
        # - "auto"    : use _infer_time_axis result from QueryParser (default; current behavior).
        # - "session" : force axis to SESSION (only query session_tree). ANCHOR queries
        #               degrade to full pool because anchor semantics are event-time only.
        # - "event"   : force axis to EVENT (only query event_tree). ANCHOR unchanged
        #               (already hard-coded to event tree).
        # - "none"    : skip the entire temporal filtering block (vector ranking only).
        ...
        # Anchor resolution: only initialize if use_anchor is True.
        ...
        # Per-query latency tracking for paper experiments.
        # Each entry: {user_id, q_id, parse_source, t_parse, t_filter, t_rank, t_graph, t_total}.
    ...
    def _expand_graph_for_blocks(
        ...
        try:
            ...
            if self.graph_include_relations and self.graph_person_relations:
                ent_names = set()
                for rel in relations or []:
                    src = str(rel.get("source") or "").strip()
                    dst = str(rel.get("destination") or "").strip()
    ...
    def _filter_by_metadata(
        ...
        if directive.use_interval_tree and directive.time_constraint.type != "NONE":
            ...
            # Force the axis according to CLI flag, regardless of what
            # QueryParser inferred. ANCHOR is special-cased below.
            if self.axis_mode == "none":
                # Skip temporal filtering entirely → fall back to full pool.
                mx.logger.info(
                ...
                if self.axis_mode == "session":
                    # Anchors are real-world events by definition; session_tree
                    # has no meaningful semantics for anchor resolution.
                    # Honest fallback: treat as no-temporal-filter for this query.
                    mx.logger.info(
    ...
    def _score_and_rank(
        ...
        Returns:
            Tuple of (rankings, similarity_map, target_boxes, query_time_meta, timing_info).
            timing_info: {parse_source, t_parse, t_filter, t_rank, t_graph, search_latency, t_total}.
            search_latency = t_filter + t_rank (excludes parse).
        """
        mx = _mx()
        ...
        # search_latency = filter + rank, excluding query parsing.
        # Tracked separately because parse cost is dominated by occasional
        # LLM fallbacks that don't reflect the cost of the retrieval algorithm
        # itself (interval-tree filter + vector ranking).
        t_search = t_filter + t_rank
        t_total = t_parse + t_search
        mx.logger.info(
            "⏱️ Timing: parse=%.3fs, filter=%.3fs, rank=%.3fs, search_latency=%.3fs, total=%.3fs",
            t_parse, t_filter, t_rank, t_search, t_total
        )

        # Record per-query latency for paper experiments. parse_source is
        # "FAST" / "LLM" / "FALLBACK" when parsing ran; "NONE" when skipped
        # (baseline mode or parsing exception).
    ...
    def run(
        ...
        for conv_idx, data in enumerate(raw_list):
            ...
            for qa_idx, qa in enumerate(data.get("qa", [])):
                ...
                rankings, _, target_boxes, query_time_meta, _ = self._score_and_rank(
        ...
        # Aggregate per-query latencies for the paper. Save raw records next to
        # the retrieval JSONL so they can be re-analyzed without rerunning.
    ...
    def _print_timing_summary(self, save_path: Optional[str] = None) -> None:
        """Aggregate per-query latencies recorded in self.timing_records.

        Reports parse_source breakdown (FAST / LLM / FALLBACK / NONE) and
        mean / median / p95 / max for parse, filter, rank, and total. Also
        splits parse latency by source so the FAST regex path and LLM
        fallback can be reported separately in the paper.
        """
        import statistics
        mx = _mx()
        ...
        def _stats(values: List[float]) -> Dict[str, float]:
            ...
            sv = sorted(values)
            # p95 with nearest-rank: index = ceil(0.95 * N) - 1, clamped to [0, N-1].
        ...
        fast_parse = [r["t_parse"] for r in records if r.get("parse_source") == "FAST"]
        llm_parse = [r["t_parse"] for r in records if r.get("parse_source") == "LLM"]

        sep = "=" * 78
        mx.logger.info(sep)
        mx.logger.info("📊 Retrieval Latency Summary (n=%d)", n)
        mx.logger.info("-" * 78)
        mx.logger.info("Parse source breakdown:")
        for src in ("FAST", "LLM", "FALLBACK", "NONE"):
            cnt = source_counts.get(src, 0)
            if cnt:
                mx.logger.info("  %-9s : %5d  (%5.1f%%)", src, cnt, 100.0 * cnt / n)
        mx.logger.info("-" * 78)
        mx.logger.info("Per-stage latency (mean / median / p95 / max), seconds:")
        ...
        mx.logger.info("  search_latency : %.4f / %.4f / %.4f / %.4f   (= filter + rank, excludes parse)",
        ...
        mx.logger.info("-" * 78)
        if fast_parse:
            sf = _stats(fast_parse)
            mx.logger.info("  parse[FAST] (n=%d): mean=%.4f median=%.4f p95=%.4f max=%.4f",
                           len(fast_parse), sf["mean"], sf["median"], sf["p95"], sf["max"])
        if llm_parse:
            sl = _stats(llm_parse)
            mx.logger.info("  parse[LLM]  (n=%d): mean=%.4f median=%.4f p95=%.4f max=%.4f",
                           len(llm_parse), sl["mean"], sl["median"], sl["p95"], sl["max"])
        mx.logger.info(sep)
