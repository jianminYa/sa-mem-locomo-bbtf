                ...
                if intent in ["PLANNING", "STATIC"] and tc.type in ["RANGE", "POINT", "AFTER", "BEFORE"]:
                    ...
                    if start_str:
                        try:
                            ...
                            if tc.type == "AFTER":
                                ...
                                expanded_end = None
                                expanded_cutoff = None
                                run = lambda use_evt: _temporal_query_ids(expanded_start, None, "AFTER", use_evt)
                                ...
                                expanded_start = None
                                expanded_end = None
                                run = lambda use_evt: _temporal_query_ids(expanded_cutoff, None, "BEFORE", use_evt)
                                ...
                                expanded_cutoff = None
                                run = lambda use_evt: _temporal_query_ids(expanded_start, expanded_end, "RANGE", use_evt)

                            time_filtered_ids, mode_used = dispatch_temporal_filter(
                                axis,
                                query_event=lambda: run(True),
                                query_session=lambda: run(False),
                            )
                            mx.logger.info(
                                "🔍 %s temporal filter (axis=%s, mode=%s): %s -> %d/%d blocks",
                                intent, axis, mode_used, tc.type,
                                len(time_filtered_ids), len(pool_ids),
                            )
                        except Exception as e:
                            mx.logger.warning("⚠️ Failed to parse %s date: %s", intent, e)
                            time_filtered_ids, _ = dispatch_temporal_filter(
                                (getattr(directive, "time_axis", "BOTH_UNION") or "BOTH_UNION").upper(),
                    ...
                    if start_str and tc.type in ["RANGE", "POINT", "AFTER", "BEFORE"]:
                        try:
                            ...
                            mx.logger.info(
                                "🔍 Temporal filter (axis=%s, mode=%s): %s (expanded) -> %d/%d blocks",
                                axis, mode_used, tc.type,
                                len(time_filtered_ids), len(pool_ids),
                            )
                        except Exception as e:
