...
# P0 fix for LME-style "user talks now about past event":
# Conversational queries like "last month", "in April", "two months ago" are
# inherently fuzzy because users talk about events over a multi-week span,
# not literally within the parsed [start, end] window. Setting this env to 1
# forces every relative-time matcher to emit is_fuzzy=True, which triggers
# DateResolver.apply_buffer to widen the window (±7d / ±20d / ±90d).
# Default OFF to preserve LoCoMo behavior.
FUZZY_RELATIVE = os.environ.get("FUZZY_RELATIVE", "0") not in ("0", "", "false", "False")

# B1 LME fix (S2 — asymmetric look-back for "N months ago" / "last N months"):
# When the active question_id is in B1_RELAXED_NAGO_TARGET_QIDS, we widen the
# month-unit relative window so that gold evidence spread across [N-1, N+3]
# months back is not pruned. Disabled by default; activated only for the
# explicitly listed qids to avoid any regression on other queries.
#   - B1_RELAXED_NAGO=1                  -> apply S2 to ALL questions
#   - B1_RELAXED_NAGO_TARGET_QIDS=q1,q2  -> apply S2 only to listed qids
_B1_RELAXED_NAGO_GLOBAL = os.environ.get("B1_RELAXED_NAGO", "0") not in ("0", "", "false", "False")
_B1_RELAXED_NAGO_TARGET_QIDS = {
    s.strip() for s in os.environ.get("B1_RELAXED_NAGO_TARGET_QIDS", "").split(",") if s.strip()
}
_CURRENT_QID: Optional[str] = None


def set_current_qid(qid: Optional[str]) -> None:
    """Set the question_id of the query about to be parsed (module-level state).

    Called by retrieval_lme.py right before QueryParser.parse() so that
    matchers can selectively change behavior based on qid (used by the S2
    asymmetric look-back fix for B1 / N-months-ago window pruning).
    """
    global _CURRENT_QID
    _CURRENT_QID = qid


def _b1_relaxed_nago_active() -> bool:
    if _B1_RELAXED_NAGO_GLOBAL:
        return True
    if _B1_RELAXED_NAGO_TARGET_QIDS and _CURRENT_QID in _B1_RELAXED_NAGO_TARGET_QIDS:
        return True
    return False
...
class TimeConstraint:
    ...
    type: str = "NONE"  # "POINT", "RANGE", "AFTER", "BEFORE", "ANCHOR"， NONE
...
def _infer_time_axis(query: str, intent: str, time_kind: str) -> str:
    ...
    Priority (highest first):
      ...
      4. Both cues         -> "BOTH_UNION" (when both session and event verbs
         are present, the intent is ambiguous — use union to avoid missing
         target blocks on either axis, e.g. "I mentioned visiting a museum
         two months ago" has "mentioned"=session + "visiting"=event)
...
_RE_MONTH_DAY_NOYEAR = re.compile(
    rf"\b({_MONTHS_RE})\.?\s+(\d{{1,2}})(?:st|nd|rd|th)?\b", re.I,
)
_RE_DAY_MONTH_NOYEAR = re.compile(
    rf"\b(\d{{1,2}})(?:st|nd|rd|th)?\s+(?:of\s+)?({_MONTHS_RE})\.?\b", re.I,
)
_RE_YEAR_AFTER_COMMA = re.compile(rf",\s*({_YEAR_RE})")


def _try_extract_date_from_anchor(
    event: str, original_query: str, clause_end_pos: int, base_time: datetime,
) -> Optional[TimeMatch]:
    """Try to find a concrete date inside an anchor body.

    Three-stage strategy:
      1. Run all non-anchor matchers on the event body.
      2. If the clause was cut at a comma, peek past it for a year suffix
         (e.g. ", 2023") and retry with the year appended.
      3. Try year-less date patterns (Month Day / Day Month) using base_time.year.
    """
    # Stage 1: direct inner match
    for matcher in _MATCHERS_NO_ANCHOR:
        tm = matcher(event, base_time)
        if tm is not None:
            return tm

    # Stage 2: peek past comma for a year suffix
    rest = original_query[clause_end_pos:]
    ym = _RE_YEAR_AFTER_COMMA.match(rest)
    if ym:
        extended = event + ", " + ym.group(1)
        for matcher in _MATCHERS_NO_ANCHOR:
            tm = matcher(extended, base_time)
            if tm is not None:
                return tm

    # Stage 3: year-less date patterns with base_time.year
    y = base_time.year
    md = _RE_MONTH_DAY_NOYEAR.search(event)
    if md:
        mo = _MONTH_NUM[md.group(1).lower()]
        d = int(md.group(2))
        try:
            dd = date(y, mo, d)
            return TimeMatch(
                span=md.span(), text=md.group(), start=dd.isoformat(),
                end=dd.isoformat(), kind="ABSOLUTE", boundary="POINT",
            )
        except ValueError:
            pass
    dm = _RE_DAY_MONTH_NOYEAR.search(event)
    if dm:
        d = int(dm.group(1))
        mo = _MONTH_NUM[dm.group(2).lower()]
        try:
            dd = date(y, mo, d)
            return TimeMatch(
                span=dm.span(), text=dm.group(), start=dd.isoformat(),
                end=dd.isoformat(), kind="ABSOLUTE", boundary="POINT",
            )
        except ValueError:
            pass
...
def _m_anchor(q: str, base_time: datetime) -> Optional[TimeMatch]:
    ...
    relation = _ANCHOR_RELATION.get(kw, "DURING")
    span = (m.start(), m.start() + len(clause_text))

    # Try to extract a concrete date from the anchor body.
    inner = _try_extract_date_from_anchor(event, q, span[1], base_time)
    if inner and (inner.start is not None or inner.end is not None):
        if relation == "AFTER":
            ref = inner.end or inner.start
            after_d = datetime.strptime(ref, "%Y-%m-%d").date() + timedelta(days=1)
            return TimeMatch(
                span=span, text=clause_text,
                start=after_d.isoformat(), end=None,
                kind=inner.kind, boundary="AFTER",
                anchor_event=event, anchor_relation=relation,
            )
        elif relation == "BEFORE":
            ref = inner.start or inner.end
            before_d = datetime.strptime(ref, "%Y-%m-%d").date() - timedelta(days=1)
            return TimeMatch(
                span=span, text=clause_text,
                start=None, end=before_d.isoformat(),
                kind=inner.kind, boundary="BEFORE",
                anchor_event=event, anchor_relation=relation,
            )
        else:  # DURING
            return TimeMatch(
                ...
                kind=inner.kind, boundary=inner.boundary,
                anchor_event=event, anchor_relation=relation,
            )

    # No date in anchor body — check if a better non-anchor match exists
    # elsewhere in the query. If so, yield to it by returning None.
    for matcher in _MATCHERS_NO_ANCHOR:
        alt = matcher(q, base_time)
        if alt is not None:
            return None
...
def _mk(q, m, start, end, kind, boundary, **kwargs) -> TimeMatch:
    is_fuzzy = kwargs.pop("is_fuzzy", _is_fuzzy_text(q[m.start():m.end()]))
    # P0 LME fix: when FUZZY_RELATIVE is on, treat every RELATIVE_* match and
    # bare-month / month-year matches as fuzzy so apply_buffer widens the
    # window. Helps "last month" / "in April" / "two months ago" recall the
    # session-time blocks that sit ±N days outside the literal window.
    if FUZZY_RELATIVE and not is_fuzzy:
        if kind in ("RELATIVE_POINT", "RELATIVE_RANGE"):
            is_fuzzy = True
    return TimeMatch(
        span=m.span(),
...
# Bare month without year: "in June", "in October" (uses base_time year).
_RE_MONTH_ONLY = re.compile(
    rf"\b(?:in|during)\s+({_MONTHS_RE})\b(?!\s*,?\s*{_YEAR_RE})",
    re.I,
)


def _m_month_only(q, bt):
    m = _RE_MONTH_ONLY.search(q)
    if not m:
        return None
    mo = _MONTH_NUM[m.group(1).lower()]
    y = bt.year
    # P0 LME fix: bare month "in April" is conversationally fuzzy — the user
    # often says it days/weeks after the actual events, so target session_time
    # spills outside the literal calendar month.
    return _mk(q, m, date(y, mo, 1), _eom(y, mo), "ABSOLUTE", "RANGE",
               is_fuzzy=FUZZY_RELATIVE)


# N-ago / last-N
_RE_N_AGO = re.compile(
    r"\b(a|an|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|\d+)\s+(day|week|month|year|quarter)s?\s+ago\b", re.I,
)
_RE_LAST_N = re.compile(
    r"\b(?:the\s+)?(?:last|past|previous)\s+(a|an|few|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|\d+)\s+(day|week|month|year|quarter)s?\b",
    re.I,
)
...
def _m_n_ago(q, bt):
    ...
    if unit == "week":
        ...
        return _mk(q, m, s, e, "RELATIVE_RANGE", "RANGE")
    if unit == "month":
        # B1 S2: asymmetric look-back for the listed qids.
        # Window = [q_date - (N+3)M, q_date - max(N-1,0)M] to recover gold
        # evidence that spreads across multiple months around the literal
        # "N months ago" anchor (LongMemEval users describe past events
        # imprecisely, e.g. "two months ago" can refer to events 1-5 months
        # back). Default behavior unchanged (single calendar month).
        if _b1_relaxed_nago_active():
            start_anchor = _month_shift(today.replace(day=15), -(n + 3))
            end_anchor = _month_shift(today.replace(day=15), -max(n - 1, 0))
            s = date(start_anchor.year, start_anchor.month, 1)
            e = _eom(end_anchor.year, end_anchor.month)
            return _mk(q, m, s, e, "RELATIVE_RANGE", "RANGE")
...
def _m_last_n(q, bt):
    ...
    if unit == "week":
        ...
        return _mk(q, m, s, today, "RELATIVE_RANGE", "RANGE")
    if unit == "month":
        # B1 S2: same asymmetric look-back logic as _m_n_ago (treat
        # "last N months" identically to "N months ago").
        if _b1_relaxed_nago_active():
            start_anchor = _month_shift(today.replace(day=15), -(n + 3))
            s = date(start_anchor.year, start_anchor.month, 1)
            return _mk(q, m, s, today, "RELATIVE_RANGE", "RANGE")
...
_MATCHERS = [
    ...
    _m_month_only,         # in June (bare month, uses base_time year)
...
_MATCHERS_NO_ANCHOR = [m for m in _MATCHERS if m is not _m_anchor]
