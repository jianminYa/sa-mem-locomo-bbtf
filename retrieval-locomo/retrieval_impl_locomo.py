...
class SimpleRetriever:
    ...
    def __init__(
        ...
        # Each entry: {user_id, q_id, t_rank, t_graph, t_total}.
    ...
    def _score_and_rank(self, user_id: Any, qa: Dict[str, Any]):
        ...
        t_rank_start = _time.perf_counter()
        ...
        store.flush()
        t_rank = _time.perf_counter() - t_rank_start
        t_total = t_rank  # Baseline has no parse/filter, so total = rank

        # Record per-query latency for paper experiments.
            ...
            for qa_idx, qa in enumerate(data.get("qa", [])):
                ...
                if self.graph_expand and rankings.get("content_event_topic_kw"):
                    ...
                    if self.timing_records:
                        ...
                        rec["t_graph"] = t_graph
                        rec["t_total"] = float(rec.get("t_rank", 0.0) or 0.0) + t_graph
    ...
    def _print_timing_summary(self, save_path: Optional[str] = None) -> None:
        ...
        Reports mean / median / p95 / max for rank and total. Since baseline
        mode has no query parsing or filtering, parse source is always "BASELINE".
        """
        import statistics
        mx = _mx()
        ...
        mx.logger.info("  %-9s : %5d  (%5.1f%%)", "BASELINE", n, 100.0)
        mx.logger.info("-" * 78)
        mx.logger.info("Per-stage latency (mean / median / p95 / max), seconds:")
        mx.logger.info("  parse          : 0.0000 / 0.0000 / 0.0000 / 0.0000  (baseline: no query parsing)")
        mx.logger.info("  filter         : 0.0000 / 0.0000 / 0.0000 / 0.0000  (baseline: no filtering)")
        ...
        mx.logger.info("  search_latency : %.4f / %.4f / %.4f / %.4f   (= filter + rank, excludes parse)",
                       s_rank["mean"], s_rank["median"], s_rank["p95"], s_rank["max"])
        mx.logger.info("  TOTAL          : %.4f / %.4f / %.4f / %.4f   (= rank + graph)",
