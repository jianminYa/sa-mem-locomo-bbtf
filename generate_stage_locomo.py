...
from generate_prompts_locomo import PROMPT_QA_ANSWER as PROMPT_QA_ANSWER_LOCOMO
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional explicit output directory. If set, overrides the directory derived from --run-id.",
    )
    ...
    parser.add_argument("--final-content-file", type=str, default=None,
                       help="Path to final_boxes_content.jsonl for loading memory contexts. Defaults to OUTPUT_DIR/final_boxes_content.jsonl.")
    # 添加一个用于区分不同生成实验的后缀参数
    ...
    # Allow an explicit output directory override for subset / ad hoc runs.
    # Keep RUN_ID for metadata, but redirect all derived paths to the requested folder.
    if args.output_dir:
        explicit_output_dir = os.path.abspath(args.output_dir)
        mx.Config.OUTPUT_DIR = explicit_output_dir
        mx.Config.FINAL_CONTENT_FILE = os.path.join(mx.Config.OUTPUT_DIR, "final_boxes_content.jsonl")
        mx.Config.VECTOR_DIR = os.path.join(mx.Config.OUTPUT_DIR, "vector_store")
        mx.Config.SIMPLE_RETRIEVAL_JSONL = os.path.join(mx.Config.OUTPUT_DIR, "simple_retrieval.jsonl")
        mx.Config.SIMPLE_RETRIEVAL_CSV = os.path.join(mx.Config.OUTPUT_DIR, "simple_retrieval.csv")
        mx.Config.GENERATION_RESULT_FILE = os.path.join(mx.Config.OUTPUT_DIR, "generation_results.jsonl")
        mx.Config.GENERATION_REPORT_CSV = os.path.join(mx.Config.OUTPUT_DIR, "report_generation_qa.csv")
        mx.Config.TOKEN_LOG_FILE = os.path.join(mx.Config.OUTPUT_DIR, "token_stream.jsonl")
        mx.Config.BUILD_TRACE_FILE = os.path.join(mx.Config.OUTPUT_DIR, "trace_build_process.jsonl")
        mx.Config.TIME_TRACE_FILE = os.path.join(mx.Config.OUTPUT_DIR, "time_traces.jsonl")
        mx.Config.TRACE_PROMPT_LOG_FILE = os.path.join(mx.Config.OUTPUT_DIR, "trace_prompts.jsonl")
        mx.Config.BUILD_STATS_FILE = os.path.join(mx.Config.OUTPUT_DIR, "build_stats.jsonl")
        mx.Config.GEN_SUMMARY_FILE = os.path.join(mx.Config.OUTPUT_DIR, "generation_metrics_summary.jsonl")
        mx.Config.BUILD_CHECKPOINT_FILE = os.path.join(mx.Config.OUTPUT_DIR, "build_checkpoint.json")
        mx.Config.TOPIC_CACHE_FILE = os.path.join(mx.Config.OUTPUT_DIR, "topic_cache.json")
        os.makedirs(mx.Config.OUTPUT_DIR, exist_ok=True)
        os.makedirs(mx.Config.VECTOR_DIR, exist_ok=True)

    if args.final_content_file:
        mx.Config.FINAL_CONTENT_FILE = os.path.abspath(args.final_content_file)
    ...
    if args.limit_conversations is not None:
        ...
        mx.logger.info(f"ℹ️ LIMIT_CONVERSATIONS set to {args.limit_conversations}")
    ...
    mx.logger.info(f"Graph expanded topk: {getattr(mx.Config, 'GRAPH_CONTEXT_EXPANDED_TOPK', None)}")
    mx.logger.info(f"Graph expanded min_score: {getattr(mx.Config, 'GRAPH_CONTEXT_EXPANDED_MIN_SCORE', None)}")
